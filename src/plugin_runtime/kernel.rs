...
impl<'a> Kernel<'a> {
    pub fn builder(config: Config) -> KernelBuilder<'a> {
    ...
    pub fn get_plugins(&self) -> &[Plugin<'a>] {
        &self.plugins[..]
    }
}
...
pub struct KernelBuilder<'a> {
    config: Config,
    injections: Vec<(Plugin<'a>, InjectionTarget)>,
}
...
impl<'a> KernelBuilder<'a> {
    ...
    pub fn inject_plugin(&mut self, plugin: Plugin<'a>, target: InjectionTarget) -> &mut Self {
        self.injections.push((plugin, target));
        self
    }

    pub fn build(&mut self) -> Result<Kernel<'a>, failure::Error> {
    ...
    fn start_plugins(plugins: Vec<RawPlugin>) -> Result<Vec<Plugin<'static>>, failure::Error> {
